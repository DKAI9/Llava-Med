...
- [Med-VQA Extensions (This Repo)](#med-vqa-extensions-this-repo)
- [Install](#install)
...
## Med-VQA Extensions (This Repo)

This repository includes additional scripts outside the upstream `LLaVA-Med/` package for training and evaluating
LLaVA-Med on the SLAKE visual question answering dataset. The new entry points live at the repository root
(e.g. `train_llava_med.py`, `eval_llava_med.py`, `slake_to_llava_sft.py`, `test.py`) and use helper modules in
`data/`, `models/`, `utils/`, and `callbacks/`.

To run these scripts, install the package dependencies from the repository root:

```bash
pip install -e "LLaVA-Med[train,eval]"
```

### SLAKE Setup

By default the scripts expect a local SLAKE directory in `SLAKE/` with JSON splits (`train.json`,
`validation.json`, `test.json`) and an `img/` folder. You can instead fetch from HuggingFace with
`--data_source hf` (requires `datasets` installed).
...
Key flags include:
- `--data_source hf|local` to switch dataset loading.
- `--mask_mode {none,crop,masked}` to apply segmentation masks.
- `--use_lora` and LoRA hyperparameters for efficient finetuning.
- `--run_final_eval` / `--final_eval_split` for post-training evaluation.
...
The evaluation script separates OPEN vs CLOSED questions, supports optional vocabulary routing for
closed-form answers, and can log to TensorBoard with `--tb`.

### Convert SLAKE to LLaVA SFT Format
...
Use the helper script to verify model loading, tokenizer image token alignment, and a tiny generate pass:

```bash
python test.py --model_id microsoft/llava-med-v1.5-mistral-7b --image /path/to/image.png
```
